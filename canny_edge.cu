...
double one_canny(char *infilename, float sigma, float tlow, float thigh)
{
	...
	double time_spent;			/* Store time in here */
	begin = clock();
	...
	if (infilename == NULL || sigma < 0 || tlow < 0 || thigh < 0) return -1;
	/****************************************************************************
	* Read in the image. This read function allocates memory for the image.
	****************************************************************************/
	if (VERBOSE) printf("\n\nEDGE DETECTOR USING CPU ONLY");
	...
	****************************************************************************/
	sprintf_s(outfilename, sizeof(outfilename), "%s_s_%3.2f_l_%3.2f_h_%3.2f_one.pgm", infilename,
		sigma, tlow, thigh);
